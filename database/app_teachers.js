...
//2. processing insert new teachers form data
function startTeacherInsertingProcess() {
    ...
    var storageRef = firebase.storage().ref("clients/"+ currentAdmin +"/app_teacher/" + imageName);
    ...
    uploadTask.on('state_changed', function(snapshot){
        ...
        switch (snapshot.state) {
          ...
          insertTeacher(fullName, designation, experiance, phone1, phone2, email1, email2, downloadURL, currentTime);
...
function insertTeacher(fullName, designation, experiance, phone1, phone2, email1, email2, downloadURL, time) {

  	var newTeacherRef = firebase.database().ref("clients/"+ currentAdmin +"/app_teachers/teachers");

  	// Time should be replaces by teachers id
  	newTeacherRef.child(time).set({
    		full_name: fullName,
    ...
    .then(function() {
        ...
        swal({
              title: "Data inserted",
              text: "Teacher data has been inserted successfully !",
              type: "success",
...
function getAllTeachers(clientId) {

    var teacherRootRef = firebase.database().ref().child("clients/"+ clientId +"/app_teachers/teachers");
    teacherRootRef.on("child_added", data => {
        ...
        var rating = data.child("rating").val();
        var downloadURL = data.child("image_url").val();

        displayTeachers(fullName, designation, experiance, phone1, phone2, email1, email2, downloadURL, rating);
...
//5. Displaying news and events in admin page
function displayTeachers(fullName, designation, experiance, phone1, phone2, email1, email2, downloadURL, rating) {
...
$("#cancleBtn").click(function() {
  ...
  $("#addNewTeacherModal").modal("toggle");
});
